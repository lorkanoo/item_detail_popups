...
use crate::{
    ...
    thread::open_link_thread,
};

use super::util::ui::{process_ui_actions_for_vec, UiAction};

impl Context {
    pub fn render_pinned_popups(&mut self, ui: &Ui) {
        let mut ui_actions = vec![];
        for (i, popup) in self.ui.pinned_popups.iter_mut().enumerate() {
            Self::render_pinned_popup(ui, &mut ui_actions, i, popup);
        }
    ...
    fn process_pinned_popups_actions(&mut self, mut ui_actions: Vec<UiAction>) {
        process_ui_actions_for_vec(&mut self.ui.pinned_popups, &mut ui_actions);
